...
# Source MeLSI functions (adjust path as needed)
# source("R/melsi_robust.R")
...
metric_weights <- diag(melsi_results$M_ensemble)
taxon_names <- colnames(X_clr)
...
for (B in B_values) {
    ...
    start_time <- Sys.time()
    result <- run_melsi_permutation_test(
        X_clr, y, 
...
```{r prefiltering}
# Compare with and without pre-filtering
cat("Comparing pre-filtering effects...\n")

# With pre-filtering (default)
start_time <- Sys.time()
results_with_prefilter <- run_melsi_permutation_test(
    X_clr, y, 
    n_perms = 19, 
    B = 20, 
    pre_filter = TRUE,
...
start_time <- Sys.time()
results_without_prefilter <- run_melsi_permutation_test(
    X_clr, y, 
    n_perms = 19, 
    B = 20, 
    pre_filter = FALSE,
...
# Compare results
cat("\nPre-filtering Comparison:\n")
cat(sprintf("With pre-filtering:    F = %.4f, time = %.1fs\n", 
           results_with_prefilter$F_observed, time_with_prefilter))
cat(sprintf("Without pre-filtering: F = %.4f, time = %.1fs\n", 
           results_without_prefilter$F_observed, time_without_prefilter))
